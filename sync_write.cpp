...
#define DXL3_ID                         9                   // Dynamixel#1 ID: 1
#define DXL4_ID                         11                   // Dynamixel#2 ID: 2

#define BAUDRATE                        1000000
#define DEVICENAME                      "/dev/ttyUSB2"      // Check which port is being used on your controller
                                                            // ex) Windows: "COM1"   Linux: "/dev/ttyUSB0" Mac: "/dev/tty.usbserial-*"
#define DEVICENAME1                     "/dev/ttyUSB3" 
...
int main()
{
  ...
  dynamixel::GroupSyncWrite groupSyncWrite1(portHandler1, packetHandler, ADDR_MX_GOAL_POSITION, LEN_MX_GOAL_POSITION);

  int index = 0;
  int dxl_comm_result = COMM_TX_FAIL;             // Communication result
  int dxl_comm_result1 = COMM_TX_FAIL;             // Communication result
  bool dxl_addparam_result = false;               // addParam result
  bool dxl_addparam_result1 = false;               // addParam result
  ...
  {
    printf("Succeeded to open the port0!\n");
  }
  else
  {
    printf("Failed to open the port0!\n");
  ...
  {
    printf("Succeeded to change the baudrate0!\n");
  }
  else
  {
    printf("Failed to change the baudrate0!\n");
    printf("Press any key to terminate...\n");
    getch();
    return 0;
  }
  ...
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler1, DXL3_ID, ADDR_MX_TORQUE_ENABLE, TORQUE_ENABLE, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS)
  {
    printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
  }
  ...
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler1, DXL4_ID, ADDR_MX_TORQUE_ENABLE, TORQUE_ENABLE, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS)
  {
    printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
  }
  ...
  while(1)
  {
    ...
    dxl_addparam_result1 = groupSyncWrite1.addParam(DXL3_ID, param_goal_position);
    ...
    dxl_addparam_result1 = groupSyncWrite1.addParam(DXL4_ID, param_goal_position);
    if (dxl_addparam_result1 != true)
    {
  ...
  dxl_comm_result1 = packetHandler->write1ByteTxRx(portHandler1, DXL3_ID, ADDR_MX_TORQUE_ENABLE, TORQUE_DISABLE, &dxl_error);
  ...
  dxl_comm_result1 = packetHandler->write1ByteTxRx(portHandler1, DXL4_ID, ADDR_MX_TORQUE_ENABLE, TORQUE_DISABLE, &dxl_error);
  if (dxl_comm_result1 != COMM_SUCCESS)
  {
    printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result1));
  }
